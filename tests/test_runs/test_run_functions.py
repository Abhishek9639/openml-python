...
class TestRun(TestBase):
    ...
    def _rerun_model_and_compare_predictions(
        ...
        # Assert the task is a valid supervised task type
    ...
    def _perform_run(
        ...
        # Verify XML serialization produces valid output
        ...
        if run.trace is not None:
            # Verify trace ARFF output has required structure
            arff_dict = run.trace.trace_to_arff()
            assert isinstance(arff_dict, dict)
            assert "attributes" in arff_dict
            assert "data" in arff_dict
            assert len(arff_dict["data"]) > 0
        ...
        # Verify downloaded run has correct attributes
        assert downloaded.run_id == run_.run_id
        assert downloaded.flow_id == run.flow_id
        assert downloaded.task_id == task_id
        assert downloaded.setup_id is not None
        return run
    ...
    def _run_and_upload(
        ...
        # Assert runtime timing keys are present before detailed check
        assert "usercpu_time_millis" in run.fold_evaluations
        assert "wall_clock_time_millis" in run.fold_evaluations
